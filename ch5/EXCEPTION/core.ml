...
let rec eval_exp exp env cont =
  ...
  | ExceptionExp (exp, handler) ->
     eval_exp exp env (ExceptionCont (handler, env, cont))
  | RaiseExp exp ->
     eval_exp exp env (RaiseCont cont)
...
and apply_cont cont exp_val =
  ...
  | ApplySndCont (proc_val, cont) ->
     (match proc_val with
      | ProcVal (body, p_env) ->
         eval_exp body (exp_val :: !p_env) cont
  ...
  | ExceptionCont (handler, env, cont) ->
     apply_cont cont exp_val
  | RaiseCont cont ->

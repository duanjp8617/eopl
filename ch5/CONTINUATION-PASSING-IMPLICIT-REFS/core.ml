...
type nl_environment = (string * int) list

let nl_store = ref 0

let inc_nl_store () = nl_store := (!nl_store + 1)
                    
type nl_expression =
  | NlConstExp of int * Ploc.t
  | NlDiffExp of nl_expression * nl_expression * Ploc.t
  | NlIsZeroExp of nl_expression * Ploc.t
  | NlIfExp of nl_expression * nl_expression * Ploc.t
  | NlVarExp of int * Ploc.t
  | NlLetExp of int * nl_expression * nl_expression * Ploc.t
  | NlProcExp of int * nl_expression * Ploc.t
  | NlApplyExp of nl_expression * nl_expression * Ploc.t
  | NlProcDefExp of int * int * nl_expression * Ploc.t
  | NlLetRecExp of (nl_expression list) * nl_expression * Ploc.t
  | NlLetDefExp of int * nl_expression * Ploc.t
  | NlMultiLetExp of (nl_expression list) * nl_expression * Ploc.t
  | NlSetExp of int * nl_expression * Ploc.t
